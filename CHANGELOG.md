...
- LocalRendererBounds메서드를 좀 더 정교하게 맞춤. 하지만 아직도 정확하진 않음.
- 루트 오브젝트 정렬에서 위치나 회전 둘 중 하나만 고를 수 있는 기능 추가.

## [1.6.5] -2023-02-01
- PhysicsExtension에서 버퍼 사용을 플레이 중이 아닐때도 가능하도록 변경

## [1.6.4] -2023-01-31
- MonoBehaviour에서 사용하는 EditorComment를 에셋 없이 작성할 수 있도록 변경.
  - 에셋이 생성될 경우, 내용이 복사됨
- 표준정규분포를 따라 결과를 출력하는 RandomGaussian 메서드 추가
- GetMaxElement 등에서 길이가 0인 배열을 집어넣었을때 오류가 나는 대신 default를 반환하도록 변경
